...
class AppStore extends Component {
  ...
  getSearchResults = () => {
    ...
    const searchResult = appsList.filter(items =>
        ...
        <ul className="ul-list">
          {tabsList.map(eachItem => (
            <TabItem
              ...
              updateButton={this.setActiveTab}
            />
          ))}
        </ul>
        <ul className="image-contain">
          {filteredApps.map(everyItem => (
            <AppItem key={everyItem.appId} imageElement={everyItem} />
          ))}
        </ul>
      </div>
    )
